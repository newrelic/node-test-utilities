...
function Test(directory, pkgVersions, opts = {}) {
  ...
  const uniqSoughtFiles = [...new Set(soughtFiles)]
  const allFiles = fs.readdirSync(directory).filter((name) => {
    return /.*\.js$/.test(name)
  })
  for (const file of allFiles) {
    if (!uniqSoughtFiles.includes(file)) {
      const errMsg = `Found test ${file} which is not being run! Did you forget to specify it?`
      if (opts.strict) {
  ...
  if (testPatterns && testPatterns.length > 0) {
    pkg.tests = filterTestsByPattern(pkg.tests, testPatterns)
  }
