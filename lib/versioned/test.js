...
Test.prototype.run = function run() {
  const reportMissing = () => {
    if (this.missingFiles.length) {
      // warn or throw if files which exist are not specified.
      const plural = this.missingFiles.length === 1 ? '' : 's'
      /* eslint-disable no-console */
      console.log(
        `Found ${this.missingFiles.length} test file${plural} outside of those specified:`.red
      )
      /* eslint-disable no-console */
      console.log('')
      for (const filePath of this.missingFiles) {
        /* eslint-disable no-console */
      ...
      console.log('')
      /* eslint-disable no-console */
      console.log("Include them in a test group's package.json or delete them.".red)
      if (this.strict) {
