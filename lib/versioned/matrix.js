...
function TestMatrix(tests, pkgVersions) {
  ...
  //    }
  //  }]
  this._matrix = tests ? tests.map(function(test) {
    ...
    task.packages = Object.keys(test.dependencies).map(function(pkg) {
      ...
      var pkgIterator = {
        name: pkg,
        next: 0,
        versions: pkgVersions[pkg].filter(function(v) {
          return semver.satisfies(v + '.9999', wantedVersions)
        })
      }
 ...
 */
TestMatrix.prototype.peek = function() {
 ...
 */
TestMatrix.prototype.next = function() {
  // For each suite in the test matrix, for each combination of packages, for
  // each test file.
...
TestMatrix.prototype._peekPackages = function(pkgs) {
...
TestMatrix.prototype._getNextPackages = function(pkgs) {
  // If there are no packages associated with this test suite, return nothing.
  if (!pkgs || !pkgs.length) {
    return null
  }
...
TestMatrix.prototype._calculateLength = function() {
  //  [{
  //    "packages": [{
  //      "name": "redis",            // <-- Package name.
  //      "next": 0,                  // <-- Iteration point.
