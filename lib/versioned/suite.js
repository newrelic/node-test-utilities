...
Suite.prototype.start = function(cb) {
  ...
  this.testFolders.forEach((folder) => {
    let testPackage = require(path.join(folder, 'package'))
    if (testPackage.tests) {
      testPackage.tests.forEach((test) => {
        let dependencies = Object.keys(test.dependencies)
...
Suite.prototype._runTests = function(pkgVersions, cb) {
  ...
  this.tests = this.testFolders.map((folder) =>
    new Test(folder, pkgVersions, this.opts.allPkgs)
  ).sort((firstEl, secondEl) =>
    secondEl.matrix.length - firstEl.matrix.length
  )
