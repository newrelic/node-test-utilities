...
/**
 * The test helper agent exported on `newrelic-tester.TestAgent`
 *
 * @module
 * @private
 */
...
/**
 * Extends Tap with more assertions specific to testing transactions.
 *
 * @param {Tap} tap - The Tap module as returned by `require('tap')`.
 */
TestAgent.extendTap = function extendTap(tap) {
  var addAssert = tap.Test.prototype.addAssert.bind(tap.Test.prototype)
  addAssert('transaction', 1, tapTransaction)
}
 ...
 * transaction.
 *
 * @member tap.Test#transaction
 *
 * @param {*}       tx        - The object to test as a transaction.
 * @param {string}  [message] - Optional message for the assertion.
 * @param {object}  [extra]   - Extra tap assertion options.
 *
 * @return {bool|*} Boolean `true` if the test passes. Else... whatever tap
 *  assertions return when failing.
 */
function tapTransaction(tx, message, extra) {
  if (!tx) {
    return this.fail(message || 'should have a transaction', extra)
  }
  if (!(tx instanceof Transaction)) {
    return this.type(tx, Transaction, message || 'should have a transaction')
  }

  return this.equal(
    'id: ' + tx.id,
    'id: ' + TestAgent.instance.getTransaction().id,
    message || 'should have current transaction',
    extra
  )
}
