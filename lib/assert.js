...
function _assertCLMAttrs({ segments, enabled: clmEnabled }) {
  segments.forEach((segment) => {
    ...
    if (clmEnabled) {
      if (!attrs['code.function']) {
        return { message: 'code.function should be defined.' }
      }
      if (!attrs['code.filepath']) {
        return { message: 'code.filepath should be defined.' }
      }
      if (!attrs['code.lineno']) {
        return { message: 'code.lineno should be defined.' }
      }
      if (!attrs['code.column']) {
        return { message: 'code.column should be defined.' }
      }
      if (attrs['code.function'] !== segment.name) {
        return {
          success: false,
          message: 'code.function should have the same value as the segment name.'
        }
      }
      if (!attrs['code.filepath'].endsWith(segment.filepath)) {
        return { message: 'code.filepath should end with the segment filepath.' }
      }
      if (!attrs['code.lineno'].match(/[\d]+/)) {
        return { message: 'code.lineno should be a number.' }
      }
      if (!attrs['code.column'].match(/[\d]+/)) {
        return { message: 'code.column should be a number.' }
      }
      return { success: true, message: 'CLM behaves as expected when enabled.' }
    }
    if (attrs['code.function']) {
      return { message: 'code.function should not be defined.' }
    }
    if (attrs['code.filepath']) {
      return { message: 'code.filepath should not be defined.' }
    }
    if (attrs['code.lineno']) {
      return { message: 'code.lineno should not be defined.' }
    }
    if (attrs['code.column']) {
      return { message: 'code.column should not be defined.' }
    }

    return { success: true, message: 'CLM noops when not enabled.' }
  })
}
